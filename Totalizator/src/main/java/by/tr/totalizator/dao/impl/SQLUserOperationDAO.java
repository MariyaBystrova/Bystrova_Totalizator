 ...
 * {@link by.tr.totalizator.dao.UserOperationDAO} with a proper realization of
 * all methods for the database.
 * 
...
public class SQLUserOperationDAO implements UserOperationDAO {
	...
	public boolean createUser(User user, String password) throws DAOException {
		...
		try {
			...
			try (PreparedStatement ps = con.prepareStatement(StatementUser.INSERT_INTO_USER)) {
	...
	public User editUserPersonalInfo(User user) throws DAOException {
		...
		try {
			con = connectionPool.takeConnection();
			try (PreparedStatement ps = con.prepareStatement(StatementUser.UPDATE_USER_PERSONAL_DATA)) {
				ps.setString(1, user.getFirstName());
				ps.setString(2, user.getLastName());
	...
	public boolean editUserAccountInfo(int userId, String password) throws DAOException {
		...
		try {
			con = connectionPool.takeConnection();
			try (PreparedStatement ps = con.prepareStatement(StatementUser.UPDATE_USER_ACCOUNT_DATA)) {
	...
	public User authorization(String login, String password) throws DAOException {
		...
		try {
			con = connectionPool.takeConnection();
			try (PreparedStatement ps = con.prepareStatement(StatementUser.SELECT_USER_WHERE_LOGIN_PASSWORD)) {
